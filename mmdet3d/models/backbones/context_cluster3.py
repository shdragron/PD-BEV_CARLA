...
class ContextCluster(nn.Module):
    ...
    def __init__(self, layers, embed_dims=None,
        ...
        self.embeddings_acc = embeddings_acc
        if self.embeddings_acc:
            self.patch_embed = PointRecuder(
                patch_size=in_patch_size, stride=in_stride, padding=in_pad,
                in_chans=3, embed_dim=embed_dims[0]-2)
        else:
            self.patch_embed = PointRecuder(
                patch_size=in_patch_size, stride=in_stride, padding=in_pad,
    ...
    def forward_embeddings(self, x):
        ...
        # img_w, img_h = int(img_w/2), int(img_h/2)
        ...
        print(pos.size())
        print(torch.cat([x, pos], dim=1).size())
        x = self.patch_embed(torch.cat([x, pos], dim=1))
        print(x.size())
        return x
    def forward_embeddings_acc(self, x):
        _, c, img_w, img_h = x.shape
        img_w, img_h = int(img_w/4), int(img_h/4)
        range_w = torch.arange(0, img_w, step=1) / (img_w - 1.0)
        range_h = torch.arange(0, img_h, step=1) / (img_h - 1.0)
        fea_pos = torch.stack(torch.meshgrid(range_w, range_h, indexing='ij'), dim=-1).float()
        fea_pos = fea_pos.to(x.device)
    ...
    def forward(self, x):
        # input embedding
        if self.embeddings_acc:
            x = self.forward_embeddings_acc(x)
        else:
