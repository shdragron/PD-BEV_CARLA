...
class S3DISData(object):
    ...
    def get_infos(self, num_workers=4, has_label=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            ...
            pts_instance_mask = np.load(pts_instance_mask_path).astype(int)
            pts_semantic_mask = np.load(pts_semantic_mask_path).astype(int)
...
class S3DISSegData(object):
    ...
    def __init__(self,
        ...
        self.cat_id2class = np.ones((self.all_ids.shape[0],), dtype=int) * \
            self.ignore_index
